...
int filter_files(const char *pathname,
  ...
  if (filter->uid != -1 || filter->gid != -1 || filter->size != -1) {
    ...
    if (lstat(normalize_path(pathname, entry->d_name), &file_stat) == -1) {
      perror("lstat");
      // leczb: It's not incorrect, but I'm not sure it's a good idea to
      //        exit the program here.
      //        It wasn't defined in the original task description,
      //        but I would expect a return 0 here instead.
      exit(EXIT_FAILURE);
    }
...
// leczb: This is very nicely factored out into a separate function.
void parse_opts_and_filter(int argc, char *argv[], FILTER *filter,
  ...
  // leczb: These are not gonna work, as you have declared filter.d_type
  //        and the rest as unsigned int. Print their value after these
  //        initializations to see what actually gets stored in them.
  filter->d_type = -1;
  filter->uid = -1;
  filter->gid = -1;
  filter->size = -1;
 while ((opt = getopt(argc, argv, "n:t:o:g:s:d:")) != -1) {
    switch(opt) {
      case 'n':
        ...
        // leczb: I don't think you need this check. getopt() should check this
        //        for you.
        if (name_len > 0) {
      ...
      case 't':
        ...
        // leczb: Just a minor optimization comment: you can add "break;"
        //        statements after each if to bail out and skip over
        //        the rest.
